...
### Is the discount a total of discount on the bill (regardless of entry point, i.e. Sticky or manually applied on the POS), or should it just be the Sticky discount?
...
Sticky doesn't have support for discounts per `cart` item. The best approach is setting `productPrice` as a value before any discounts, e.g. 120 in the previous question.
