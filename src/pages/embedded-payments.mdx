...
Add more URL parameters to the end of your **Base URL**.
 ...
 - `cpaMaxLength`: Limit the number of times a subscription occurs (Integer, optional)
 - `cpaInPeriod`: Start date in unix time (seconds) (Integer, optional)
...
```
sha512_in_my_language('total=1&currency=USD&userPaymentId=REF1', 'private-abc-def')
```
...
## Respond to a successful payment in your frontend
...
## Check a payment status in your backend
...
You will receive a JSON object. The important key is `sessionPaidAt`. When this is an integer, the payment was successful and `sessionPaidAt` is a unix timestamp. When this is null and `sessionFailedAt` is an integer, the payment was not successful. We recommend using a simple assert:
...
Add flow step `Set discount (value)` *or* `Set discount (%)` then use URL parameters:

 - `setDiscountPercentage`: 20 = 20%

 - `setDiscountAbsolute`: Integer, 155 = 1 dollar and 55 cents

## QR code

Our API will happily generate a QR code for you and return PNG data with HTTP `Content-Type` `image/png`, making it easy to use in a HTML `<img />` tag.

`GET` `https://sticky.to/v2/qr/[URL_HERE]`

Be sure to URL-encode your URL parameter:

```
render () {
  return (
    <img src={`https://sticky.to/v2/qr/${encodeURIComponent('https://sticky.to/go/flow/123?total=1')}`} />
  )
}
```
...
You can update the `total` of a payment with a `PATCH`:

```
curl 'https://sticky.to/v1/payments/[PAYMENT_ID_HERE]' \
  -X 'PATCH' \
...
```
curl 'https://sticky.to/v2/trigger/capture-later' \
  -H 'Accept: application/json' \
  -H 'Authorization: Bearer [YOUR_PRIVATE_KEY]//' \
  -H 'Content-Type: application/json' \
  --data-raw '{"paymentId":"[PAYMENT_ID_HERE]"}';
```
